...
public class TopDonationLabel extends GenericLabel {
    ...
    public void onDonation(@Nullable RichMessageEvent e) {
        ...
        Thread.ofVirtual().start(() -> {
            ...
            for (Donation d : e.donations) {
                try {
                    double convertedAmount = Currencies.convertCurrency(d.amount * d.count, d.currency, Currencies.baseCurrency);
    ...
    private void updateText() {
        if (this.topDonator == null) {
            ...
            String prefix = HtmlEscape.escapeHtml5(this.settings().getString("text.prefix")).replace(" ", "&nbsp;");
            String suffix = HtmlEscape.escapeHtml5(this.settings().getString("text.suffix")).replace(" ", "&nbsp;");
