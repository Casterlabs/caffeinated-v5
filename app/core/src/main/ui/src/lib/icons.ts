	...
	if (iconCache[icon]) {
		...
		(async () => {
			try {
				...
				switch (iconType) {
					case 'outline': {
						svg = await fetch(`/images/icons/outline/${iconPath}.svg`).then((res) => {
							if (res.ok) {
								return res.text();
							} else {
					...
					case 'external': {
						svg = await fetch(`/images/icons/external/${iconPath}.svg`).then((res) => {
							if (res.ok) {
								return res.text();
							} else {
								throw 'Status: ' + res.status;
				...
				element.innerHTML = iconCache[icon] = '<div class="bg-red-500 h-full w-full text-white" title="MISSING ICON">X</div>'; // Visual error.
